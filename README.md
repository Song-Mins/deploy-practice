...
## ✏️ 워크플로우에서 전에 실행중인 애플리케이션 종료시 안전하게 종료
전에 실행중인 애플리케이션을 종료하고새로운 버전을 실행할때 그냥 종료하게 되면 처리중인 요청이 있을경우 요청을 처리하지 못한다.
따라서, transfer-jar-cicd, clone-build-cicd 브랜치에서는 kill -15 를 이용해서 우선종료를 하고 15초 기다린 다음 종료가 안되면 kill -9 으로 강제종료 하였고
pull-image 에서는 ~로 컨테이너를 우선종료 하고 종료에 실패하면 ~하였다.
